...
* [`src/clj/twitpad_mod/`](src/clj/twitpad_mod/): Backend and middleware source files (Clojure,
[Compojure](https://github.com/weavejester/compojure))
* [`src/clj/twitpad_mod/`](src/clj/twitpad_mod/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/twitpad_mod/`](src/cljs/twitpad_mod/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/twitpad_mod/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/twitpad_mod/`](test/cljs/twitpad_mod/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `twitpad-mod` project root directory
...
in the [`src/clj/twitpad_mod/`](src/clj/twitpad_mod/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/twitpad_mod/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns twitpad-mod.example
  (:require [twitpad-mod.config :as config])
...
```sh
port=2000 java -jar target/twitpad-mod.jar
```
